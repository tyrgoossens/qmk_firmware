// This is the canonical layout file for the Quantum project. If you want to add another keyboard,
// this is the style you want to emulate.

#include "planck.h"
#ifdef BACKLIGHT_ENABLE
  #include "backlight.h"
#endif

// Each layer gets a name for readability, which is then used in the keymap matrix below.
// The underscores don't mean anything - you can have a layer called STUFF or any other name.
// Layer names don't all need to be of the same length, obviously, and you can also skip them
// entirely and just use numbers.
#define _QW 0
#define _CM 1
#define _DV 2
#define _LW 3
#define _RS 4
#define _MA 5

enum macro_id {
  M_LED = 0,
  M_PASS,
  M_PASS2,
  M_SRV,
  M_BREAK,
  M_VI,
  M_TERM,
  M_ITM
};

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {

/* Qwerty
 * ,-----------------------------------------------------------------------------------.
 * | Esc  |   Q  |   W  |   E  |   R  |   T  |   Y  |   U  |   I  |   O  |   P  | Bksp |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * | Ctrl |   A  |   S  |   D  |   F  |   G  |   H  |   J  |   K  |   L  |   ;  |Enter |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * | Shift|   Z  |   X  |   C  |   V  |   B  |   N  |   M  |   ,  |   .  |   /  |Shift |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Macro| Ctrl |  GUI | Alt  | Lower|    Space    |Raise | Left | Down |  Up  |Right |
 * `-----------------------------------------------------------------------------------'
 */
[_QW] = {
  {KC_ESC,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_BSPC},
  {KC_LCTL, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_ENT },
  {FUNC(2), KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, FUNC(3)},
  {MO(_MA), KC_LCTL, KC_LGUI, KC_LALT, MO(_LW), FUNC(1), KC_NO,   MO(_RS), KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT}
},

/* Colemak
 * ,-----------------------------------------------------------------------------------.
 * | Tab  |   Q  |   W  |   F  |   P  |   G  |   J  |   L  |   U  |   Y  |   ;  | Bksp |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * | Esc  |   A  |   R  |   S  |   T  |   D  |   H  |   N  |   E  |   I  |   O  |  "   |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * | Shift|   Z  |   X  |   C  |   V  |   B  |   K  |   M  |   ,  |   .  |   /  |Enter |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Brite| Ctrl | Alt  | GUI  |Lower |    Space    | Raise| Left | Down |  Up  |Right |
 * `-----------------------------------------------------------------------------------'
 */
[_CM] = {
  {KC_TAB,  KC_Q,    KC_W,    KC_F,    KC_P,    KC_G,    KC_J,    KC_L,    KC_U,    KC_Y,    KC_SCLN, KC_BSPC},
  {KC_ESC,  KC_A,    KC_R,    KC_S,    KC_T,    KC_D,    KC_H,    KC_N,    KC_E,    KC_I,    KC_O,    KC_QUOT},
  {KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_K,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_ENT },
  {M(0),    KC_LCTL, KC_LALT, KC_LGUI, MO(_LW), KC_SPC,  KC_SPC,  MO(_RS), KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT}
},

/* Dvorak
 * ,-----------------------------------------------------------------------------------.
 * | Tab  |   "  |   ,  |   .  |   P  |   Y  |   F  |   G  |   C  |   R  |   L  | Bksp |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * | Esc  |   A  |   O  |   E  |   U  |   I  |   D  |   H  |   T  |   N  |   S  |  /   |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * | Shift|   ;  |   Q  |   J  |   K  |   X  |   B  |   M  |   W  |   V  |   Z  |Enter |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Brite| Ctrl | Alt  | GUI  | Lower|    Space    |Raise | Left | Down |  Up  |Right |
 * `-----------------------------------------------------------------------------------'
 */
[_DV] = {
  {KC_TAB,  KC_QUOT, KC_COMM, KC_DOT,  KC_P,    KC_Y,    KC_F,    KC_G,    KC_C,    KC_R,    KC_L,    KC_BSPC},
  {KC_ESC,  KC_A,    KC_O,    KC_E,    KC_U,    KC_I,    KC_D,    KC_H,    KC_T,    KC_N,    KC_S,    KC_SLSH},
  {KC_LSFT, KC_SCLN, KC_Q,    KC_J,    KC_K,    KC_X,    KC_B,    KC_M,    KC_W,    KC_V,    KC_Z,    KC_ENT },
  {KC_LCTL, KC_LCTL, KC_LALT, KC_LGUI, MO(_LW), KC_SPC,  KC_SPC,  MO(_RS), KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT}
},

/* Lower
 * ,-----------------------------------------------------------------------------------.
 * |   `  |   1  |   2  |   3  |   4  |   5  |   6  |   7  |   8  |   9  |   0  | Del  |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * | Tab  |   6  |   7  |   8  |   9  |   0  |      |   _  |   =  |   [  |   ]  |  #   |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * | CAPS |   \  |      |      |      |      |      |Qwerty|      |      |      |Enter |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Brite| Ctrl | Alt  | GUI  |      |    Space    |      | Home | PgDn | PgUp | End  |
 * `-----------------------------------------------------------------------------------'
 */
[_LW] = {
  {KC_GRAVE, KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_DEL },
  {KC_TAB,   KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_NO,   KC_UNDS, KC_EQL,  KC_LBRC, KC_RBRC, KC_BSLS},
  {KC_CAPS,  KC_NUBS, KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   DF(_QW), KC_NO,   KC_NO,   KC_NO,   KC_TRNS},
  {M(0),     KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_HOME, KC_PGDN, KC_PGUP, KC_END}
},

/* Raise
 * ,-----------------------------------------------------------------------------------.
 * |   ~  |   !  |   "  |   £  |   $  |   %  |   ^  |   &  |   *  |   (  |   )  | Del  |
 * |------+------+------+------+------+-------------+------+------+------+------+------|
 * | Esc  |   @  |   '  |      |      |      |      |   -  |   +  |   {  |   }  |  ~   |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * | Shift|   |  |      |      |      |      |      |Qwerty|      |      |      |Enter |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Ctrl | Ctrl | Alt  | GUI  |      |    Space    |      | Next | Vol- | Vol+ | Play |
 * `-----------------------------------------------------------------------------------'
 */
[_RS] = {
  {KC_TILD, KC_EXLM,       KC_AT,   KC_HASH, KC_DLR,  KC_PERC, KC_CIRC, KC_AMPR, KC_ASTR, KC_LPRN, KC_RPRN, KC_DEL },
  {KC_TAB,  LSFT(KC_QUOT), KC_QUOT, KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_MINS, KC_PLUS, KC_LCBR, KC_RCBR, KC_PIPE},
  {KC_LSFT, LSFT(KC_NUBS), KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO,   DF(_QW), KC_NO,   KC_NO,   KC_NO,   KC_TRNS},
  {KC_TRNS, KC_TRNS,       KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_MNXT, KC_VOLD, KC_VOLU, KC_MPLY}
},

/* Macro
 * ,-----------------------------------------------------------------------------------.
 * |   `  | Pass | ITM  |      |      |  VI  | TERM |      | Break|      |  SRV | Del  |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Tab  |  F1  |  F2  |  F3  |  F4  |  F5  |  F6  |   -  |   =  |   [  |   ]  |  \   |
 * |------+------+------+------+------+------|------+------+------+------+------+------|
 * |      |  F7  |  F8  |  F9  |  F10 |  F11 |  F12 |Qwerty|      |      | Reset|Enter |
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * |      |      |      |      |      |    Macro    |ubrite|      |      |      |C-A-D |
 * `-----------------------------------------------------------------------------------'
 */
[_MA] = {
  {KC_GRV,  M(M_PASS), M(M_PASS2), KC_TRNS, KC_TRNS, M(M_VI), M(M_TERM), KC_TRNS, M(M_BREAK), KC_TRNS, M(M_SRV), KC_DEL },
  {KC_TAB,  KC_F1,     KC_F2,    KC_F3,   KC_F4,   KC_F5,   KC_F6,     KC_MINS, KC_EQL,     KC_LBRC, KC_RBRC,  KC_BSLS},
  {KC_TRNS, KC_F7,     KC_F8,    KC_F9,   KC_F10,  KC_F11,  KC_F12,    DF(_QW), KC_TRNS,    KC_TRNS, RESET,    KC_TRNS},
  {KC_TRNS, KC_TRNS,   KC_TRNS,  KC_TRNS, FUNC(4), KC_TRNS, KC_TRNS,   M(0),    KC_TRNS,    KC_TRNS, KC_TRNS,  LALT(LCTL(KC_DEL))}
}
};

const uint16_t PROGMEM fn_actions[] = {
    [1] = ACTION_LAYER_TAP_KEY(_MA, KC_SPC),      // Tap for space, hold for SpaceFN
    [2] = ACTION_MODS_TAP_KEY(MOD_LSFT, KC_CAPS),  //Shift on press, Caps on tap
    [3] = ACTION_MODS_TAP_KEY(MOD_RSFT, KC_CAPS),  //Shift on press, Caps on tap
    [4] = ACTION_BACKLIGHT_DECREASE(),
};

const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  // MACRODOWN only works in this function
      switch(id) {
        case 0:
          if (record->event.pressed) {
            register_code(KC_RSFT);
            #ifdef BACKLIGHT_ENABLE
              backlight_increase();
            #endif
          } else {
            unregister_code(KC_RSFT);
          }
        break;
        case M_PASS:
          if (record->event.pressed) {
            return MACRO( I(1), D(LSFT), T(Z), T(X), T(C), T(V), U(LSFT), T(9), T(9), T(Z), T(X), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
        case M_PASS2:
          if (record->event.pressed) {
            return MACRO( I(1), D(LSFT), T(C), T(V), T(B), T(N), U(LSFT), T(1), T(1), T(Q), T(W), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
        case M_SRV:
          if (record->event.pressed) {
            return MACRO( I(1), T(DOT), T(S), T(R), T(V), T(DOT), T(B), T(E), T(DOT), T(E), T(U), T(R), T(O), T(P), T(E), T(DOT), T(I), T(N), T(T), T(R), T(A), T(N), T(E), T(T), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
        case M_BREAK:
          if (record->event.pressed) {
            return MACRO( I(1), T(GRV), T(DOT), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
        case M_VI:
          if (record->event.pressed) {
            return MACRO( I(1), T(S), T(E), T(T), T(SPC), T(MINS), T(O), T(SPC), T(V), T(I), T(ENT), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
        case M_TERM:
          if (record->event.pressed) {
            return MACRO( I(1), T(E), T(X), T(P), T(O), T(R), T(T), T(SPC), D(LSFT), T(T), T(E), T(R), T(M), U(LSFT), T(EQL), T(X), T(T), T(E), T(R), T(M), T(SPC), T(SCLN), END);
          }  else {
            return MACRO_NONE ;
          }
        break;
      }
    return MACRO_NONE;
};
